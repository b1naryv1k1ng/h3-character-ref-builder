...
def extension_for_upload(
    slot: str, original_filename: str, content_type: str | None = None
) -> str:
    del content_type  # The file content is inspected after upload; MIME is not trusted.
    ...
    allowed = IMAGE_EXTENSIONS if slot in IMAGE_SLOTS else AUDIO_EXTENSIONS
    if extension not in allowed:
        raise InvalidMedia(
            f"Unsupported file extension {extension or '(none)'} for {slot}. "
            f"Allowed: {', '.join(sorted(allowed))}."
        )
    return extension


def validate_media_file(slot: str, path: Path, extension: str) -> None:
    ...
    if slot in IMAGE_SLOTS:
        ...
        if detected != expected_format:
            raise InvalidMedia("Uploaded image content does not match its file extension.")
        return

    if slot != "reference_audio":
        raise InvalidMedia(f"Invalid media slot: {slot!r}.")
    try:
        import av
